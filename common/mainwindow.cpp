    ...
    mode("Init")
{
    ...
    //Resize the mainwindow according to the size of the monitor
    screen = QApplication::desktop()->screenGeometry();cout<<screen.height()<<endl;
    this->setGeometry(0,0,screen.width()/2-68,screen.height());

    //Instantiate processEngine class for both frames
    procEng = new processEngine;
    procEng2 = new processEngine;

    //Connect signals comming from processEngine instance to the slot here
    ...
    //Set the position of the first frame
    cv::moveWindow(outFrame,screen.width()/2,0);

    //Disable everything related to the second frame so that the first frame is the one we choose at the beginning
    ...
    //Disable widgets for the initialization mode
    Disable_widgets();

    //Fill the combo box with some number of devices
    for(int i=-1; i<4; i++)
    {
...
/**
 * Clear the list and call reset of processEngine instance
 */
...
/**
 * Slots for the computation of computer vision process requiring two images
 */
...
/**
 * Slots allowing the calibration of the webcam
 */
